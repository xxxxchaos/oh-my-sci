...
同时会把每个 agent 的实际运行模型写入当前项目的 `.opencode/agents/*.md`。如果后续修改 provider 或 quota，运行 `bunx github:xxxxchaos/oh-my-sci#v0.1.4 configure --providers ... --quota ...` 即可重写配置。建议在安装或配置后运行 `doctor --models`，确认 agent 文件没有引用你未配置或不可用的模型，详情见 [模型配置指南](./model-setup.md)。
...
默认会删除当前项目中 omo-sci 生成的 agent/command、从 `opencode.json` 移除 `omo-sci` 插件项，并删除全局配置 `~/.config/opencode/omo-sci.jsonc`。Dubin 进化记忆目录默认保留；如需同时删除，运行 `bunx github:xxxxchaos/oh-my-sci#v0.1.4 uninstall --profile`。
...
**Q: MCP 工具找不到？**
A: 运行 `bunx github:xxxxchaos/oh-my-sci#v0.1.4 doctor` 检查环境。确保已配置 PubMed Search MCP 等必要工具。

**Q: 配置修改后不生效？**
A: omo-sci 在启动时读取配置。重启 OpenCode 会话或重新加载插件。

**Q: 某个 agent 调用了我没有配置的模型？**
A: 重新运行 `bunx github:xxxxchaos/oh-my-sci#v0.1.4 configure`，或手动检查 `.opencode/agents/*.md`。`doctor --models` 会标记 agent frontmatter 中不在 omo-sci 配置里的模型。
